...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try { await register(name, email, password); toast.success('Registered!'); navigate('/dashboard'); } 
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="relative z-10 glass p-8 w-96 space-y-4">
        ...
        <input type="text" name="name" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-2 rounded bg-black/20" required />
        ...
        <button type="submit" className="w-full py-2 bg-yellow-500 rounded hover:bg-yellow-600">Register</button>
        <p className="text-center">Already have account? <Link to="/login" className="underline">Login</Link></p>
      </form>
    </div>
  );
};
